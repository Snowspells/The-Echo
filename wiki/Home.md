...
1. **Discord Bot** — Runs in your Discord server. Handles account linking, staff role management, chat bridging, and utility commands.
2. **Web Dashboard** — A companion website where players log in with Discord to view their linked account, and where staff manage users and monitor the chat bridge.
3. **Chat Bridge** — A real-time message relay between a Discord channel and the in-game Global chat channel.

All three systems share a single SQLite database and run from a single Node.js process (`npm start`).
